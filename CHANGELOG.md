...
- **Worktree auto-follow never worked because the manifest was missing the `worktrees:read` permission.** `muxy.worktrees.list()` threw `permission denied (worktrees:read)`, silently caught in `getActiveProjectPath`, so it always fell through to the parent project's path. Added `worktrees:read` to `package.json`'s `muxy.permissions`.
- **A newly-active path with no conversation history yet (e.g. a fresh worktree) always showed as a raw/abbreviated "stale filter"** instead of picking up the `project · branch` label, because `loadProjectLabels` only resolves `git rev-parse`/worktree info for paths that already appear in conversation history. Added `ensureToplevelKnown(path)` in `src/panel/main.js`, called from both the mount-time sync and `applyFilter` (the auto-follow callback), which resolves that one path on demand and refreshes the picker.
- **`muxy.worktrees.list({ project: id })` threw `"project not found [object Object]"` on this Muxy build.** The whole options object was being stringified as the lookup key — the API expects the project id passed POSITIONALLY. `getActiveProjectPath` now tries `list(id)` first, falling back to `list({ project: id })` and `list()` for other Muxy versions.
- **A fresh worktree's "project · branch" label still didn't render even after the path was resolved on demand**, because `getPickerLabel`'s stale-filter fallback only checked `groups` (which is derived exclusively from paths with conversation history — a fresh worktree is never in it) and went straight to the raw truncated path. It now also checks `extractRepoLabel(filterValue, gitToplevelMap, worktreeInfoMap)` directly against the full maps before falling back, so a known worktree gets its proper label regardless of conversation history.

### Added
- **Auto-follow now tracks git worktrees, not just the parent project.** When Muxy's active project is a linked worktree, both the mount-time sync and the polling fallback now resolve to the worktree's own path (via `muxy.worktrees.list()` / `muxy.git.worktrees()`, matched with `isWorktreeActive`) instead of the parent project's path. Implemented in `getActiveProjectPath(muxy)` in `src/panel/project-change-listener.js`.
- **Worktrees get their own "WORKTREES" section in the picker, labeled "project · branch", instead of being mixed into PROJECTS with their raw directory name.** A worktree's own directory is often an arbitrary name (e.g. `ac-feature-x`) that doesn't indicate which project it belongs to, and lumping it into the main project list is confusing. `loadProjectLabels` now runs one extra `git rev-parse --git-common-dir` + `git branch --show-current` per unique repo toplevel (`gitWorktreeInfo` in `src/panel/main.js`, batched the same way as the existing rev-parse pass) to detect linked worktrees and their branch. `projectDisplayGroups` in `src/panel/utils.js` returns a third `worktrees` array (split out of `git`); `extractRepoLabel` takes an optional `worktreeInfoMap` and renders e.g. `myapp · feature-x`, falling back to the worktree's basename if the branch is empty (detached HEAD). `buildPickerItems`/`getPickerLabel` in `src/panel/project-picker.js` render the new `WORKTREES (N)` section between PROJECTS and PATHS. Non-worktree repos are unaffected.
